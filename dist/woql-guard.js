...
function hasDestructiveType(ast) {
    if (ast === null || ast === undefined || typeof ast !== "object") {
        return false;
    }
    if (Array.isArray(ast)) {
        for (var i = 0; i < ast.length; i++) {
            if (hasDestructiveType(ast[i]))
                return true;
        }
        return false;
    }
    ...
    for (var i = 0; i < keys.length; i++) {
        if (hasDestructiveType(ast[keys[i]]))
            return true;
    }
    return false;
}
 ...
 */
function assertReadOnly(woqlObject) {
    if (woqlObject && woqlObject.contains_update === true) {
        throw new Error("Read-only mode: query contains destructive operations and cannot be executed");
    }
